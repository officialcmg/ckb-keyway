...
Preflight the exact invoice and fee limit before showing a confirmation:

```ts
const preflight = await connection.keyway.preflightPayment({
  invoice,
  max_fee_amount: toHex(1n * SHANNONS_PER_CKB),
});

if (!preflight.routable) {
  console.log(preflight.error.code, preflight.error.retryable);
